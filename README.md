...
| `0x61DBE2` / Present | Exclusive fullscreen at a non-desktop resolution rendered black: the device is already lost when `CreateDevice` returns (`TestCooperativeLevel` = `D3DERR_DEVICENOTRESET`, every `Present` fails with `D3DERR_DEVICELOST`), and re-creating it is lost the same way. At the desktop resolution it works, but the game releases the device on alt-tab and the exclusive `CreateDevice` takes 3–5 s when it comes back. `BorderlessFullscreen=1` (default) runs every resolution in a borderless desktop-sized window with `D3DSWAPEFFECT_COPY` (device created in ~100 ms, kept across alt-tab) and presents the back buffer into an aspect-correct rectangle with black bars. |
